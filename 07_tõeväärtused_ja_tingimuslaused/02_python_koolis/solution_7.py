...
on alguses mõistlik sisestada erinevad isikukoodid ning kommenteerida vastavalt vajadusele ülearused välja, näiteks järgnevalt kasutatakse teisel real olevat
isikukoodi:
...
Täiendusi:
    ...
    Vastavalt toodud isikukoodi tutvustavale artiklile võrdle esimest kümmet numbrit ja viimast numbrit (nn. kontrollnumbrit), et teha selgeks,
    kas isikukood on üldse kehtiv. Kuna isikukoodi võtame kui sõnet, aga seal olevaid arve on vaja korrutada, peame tegema tüübiteisenduse: sõnena
    oleva arvu peame teisendama täisarvuks (funktsioon "int()").
    Koosta funktsioon, mis ise automaatselt koostab kehtivaid isikukoode. Võimatud on näiteks isikukoodid vale kontrollnumbriga, kuid ka sellised,
    mis viitavad olematule kuupäevale (algusega "3950230...", mis tähendaks 30. veebruari) vms. Vali kas "ohutu" tee (ette on antud piirid,
    mis kehtivad igal juhul) või loo sisemised sõltuvused (stiilis "kui kuu on aprill, siis maksimaalsete päevade arv on 30").
"""
...
def get_century(id_code: str) -> int:
    first = get_first_number(id_code)
    if first <= 2:
        return 1800
    elif first <= 4:
        return 1900
...
def get_year(id_code: str, century: int) -> int:
    #return century + int(id_code[1:3])
    year = century
