...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    fastest_tweet = tweets[0]
    max_rate = fastest_tweet.retweets / fastest_tweet.time if fastest_tweet.time > 0 else 0

    for tweet in tweets:
        current_rate = tweet.retweets / tweet.time if tweet.time > 0 else 0

        if current_rate > max_rate:
            max_rate = current_rate
            fastest_tweet = tweet

    return fastest_tweet
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        for tag in tweet.hash_tags:
            hash_tags[tag] = hash_tags.get(tag, 0) + tweet.retweets

    sorted_tags = sorted(hash_tags.keys(), key=lambda tag: (-hash_tags[tag], tag))
